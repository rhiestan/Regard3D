...
{
  return r3d_vl_liopdesc_new(DEFAULT_NUM_NEIGHBOURS,
...
void
r3d_vl_liopdesc_process (VlLiopDesc * self, float * desc, float const * patch)
{
  vl_index i,t ;
  ...
  for (i = 0 ; i < (signed)self->patchSize ; ++i) {
    ...
    for (t = 0 ; t < self->numNeighbours ; ++t) {
      ...
      self->neighIntensities[t] = (1.0 - wy) * (a + (b - a) * wx) + wy * (c + (d - c) * wx) ;
    }
     ...
     */
    {
      ...
      for(k = 0; k < self->numNeighbours ; ++k) {
        for(t = k + 1; t < self->numNeighbours; ++t){
          float a = self->neighIntensities[k] ;
          float b = self->neighIntensities[t] ;
...
vl_size
r3d_vl_liopdesc_get_dimension (VlLiopDesc const * self)
{
...
vl_size
r3d_vl_liopdesc_get_num_neighbours (VlLiopDesc const * self)
{
  assert(self) ;
...
float
r3d_vl_liopdesc_get_intensity_threshold (VlLiopDesc const * self)
{
  assert(self) ;
...
void
r3d_vl_liopdesc_set_intensity_threshold (VlLiopDesc * self, float x)
{
  assert(self) ;
...
double
r3d_vl_liopdesc_get_neighbourhood_radius (VlLiopDesc const * self)
{
  assert(self) ;
...
vl_size
r3d_vl_liopdesc_get_num_spatial_bins (VlLiopDesc const * self)
{
  assert(self) ;
