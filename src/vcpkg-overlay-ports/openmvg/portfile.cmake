...
vcpkg_cmake_install()
vcpkg_copy_pdbs()
# OVERLAY FIX -- see ../README.md
# Upstream passes CONFIG_PATH "lib/openMVG", but OpenMVG actually installs its CMake
# package one level deeper, in lib/openMVG/cmake. vcpkg_cmake_config_fixup therefore
# relocated the directory without accounting for that extra level, leaving both
# OpenMVGConfig.cmake's hand-rolled install root and the _IMPORT_PREFIX arithmetic in
# OpenMVGTargets.cmake short by one -- so find_package(OpenMVG) failed with
# "Cannot find openMVG include files", and the imported targets pointed at
# <prefix>/share/debug/lib/*.lib instead of <prefix>/debug/lib/*.lib.
#
# Passing the real config path lets vcpkg do the depth adjustment itself, and lands the
# package at <prefix>/share/openmvg -- exactly the layout OpenMVG's own "../.." assumes.
vcpkg_cmake_config_fixup(CONFIG_PATH "lib/openMVG/cmake")

# config_fixup only relocates the cmake/ directory, so move OpenMVG's remaining payload
# (sensor_width_camera_database.txt, webgl/) alongside it, as the old CONFIG_PATH did.
file(GLOB OPENMVG_LIB_PAYLOAD "${CURRENT_PACKAGES_DIR}/lib/openMVG/*")
if(OPENMVG_LIB_PAYLOAD)
    file(COPY ${OPENMVG_LIB_PAYLOAD} DESTINATION "${CURRENT_PACKAGES_DIR}/share/${PORT}")
endif()
file(REMOVE_RECURSE
    "${CURRENT_PACKAGES_DIR}/lib/openMVG"
    "${CURRENT_PACKAGES_DIR}/debug/lib/openMVG"
)
